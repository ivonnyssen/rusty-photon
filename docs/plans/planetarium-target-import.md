...
| P3 | `planetarium-bridge` service: Alpaca Telescope impersonation → target creation via rp (gated by milestone P3a, a sanctioned verification spike) | P3a spike built (`spikes/planetarium-bridge-p3a`): logging Alpaca Telescope with discovery responder, simulated slews, and automatic J2000-vs-JNow inference; run guide + session script + findings skeleton in [planetarium-bridge.md](../services/planetarium-bridge.md). Awaiting the real-SkySafari session; the spike is deleted once findings land | feature/planetarium-bridge-p3a |
