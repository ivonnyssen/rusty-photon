...
| P4 | ui-htmx target inbox: review pending targets, goal editing, PA override, activate/discard | In progress — design landed 2026-07-31 ([ui-htmx.md § Targets inbox](../services/ui-htmx.md#targets-inbox-targets)): fourth nav surface at `/targets`, driving rp's target tools through `rp-mcp-client` (consumer #4, per-request sessions) from the existing `rp` config block; PA field per the P4 note (blank ⇒ explicit-`null` clear, `0` explicit); stale-goal roster flags; provenance display | feature/ui-htmx-target-inbox |
