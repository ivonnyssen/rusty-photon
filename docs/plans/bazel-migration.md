...
| `aws-lc-sys` build fails on Windows under Bazel (MAX_PATH) | Hit | Three-pronged fix: (1) shortened `from_cargo` name from `"crates"` to `"cr"` — the repo name appears twice in every build-script runfiles path, saving 8 chars; (2) `build_script_data_glob = ["**"]` annotation ensures all vendored C files are materialised in Bazel's runfiles; (3) `AWS_LC_SYS_NO_JITTER_ENTROPY=1` via `build:windows --action_env` disables jitterentropy on Windows — its `tree_drbg_jitter_entropy.c` includes `jitterentropy.h` via a deep `../../../../` relative path whose un-normalised intermediate form (~280 chars) exceeds MSVC's MAX_PATH. aws-lc falls back to BCryptGenRandom for entropy. |
