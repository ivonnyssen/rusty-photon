...
**Dependabot auto-repin.** Dependabot bumps `Cargo.toml`/`Cargo.lock` but never `MODULE.bazel.lock`, so every cargo update left the Bazel crate index stale until someone repinned by hand. The `repin-bazel` workflow (`.github/workflows/repin-bazel.yml`) closes this gap: on Dependabot cargo PRs it runs the same `CARGO_BAZEL_REPIN=1 bazel mod tidy && bazel mod tidy` dance, validates the refreshed lock with `bazel build --nobuild //... --lockfile_mode=error`, and pushes the updated `MODULE.bazel.lock` back to the PR branch. The push uses the default `GITHUB_TOKEN`, whose commits do not re-trigger workflows — so the job validates in-place rather than relying on the non-required `bazel`/`parity` shadow jobs re-running; the repin only rewrites `MODULE.bazel.lock` (nothing Cargo builds), so the pre-repin `coverage` check stays valid. Non-Dependabot dep changes still repin by hand per CLAUDE.md rule 10.
...
- **Full coverage cutover:** `bazel coverage` is required and uploads the
  ...
  coverage job (now nightly) uploads `cargo-<pkg>` as a cross-check.
