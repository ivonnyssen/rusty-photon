...
10. **Doctor owns the TLS lifecycle; certs are generated by default; TLS is
    on.** The shared `ServerConfig` carries `tls` and `auth` for **every**
    service — all 18, same knobs, same shape. Four parts:
    ...
    **(d) TLS is on by default** — meaning **doctor's generated config enables
    it**, not that the serde default becomes `Some(tls)`. The distinction is
    load-bearing: packages start services at install (`WantedBy=multi-user.target`,
    WiX `Start="install"`) which is *before* any doctor run, so a serde default
    of "on" would leave every fresh install unable to start for want of certs
    that do not exist yet. It would also break every BDD and ConformU test that
    hand-writes a config omitting `tls`. So: **absent `tls` still means plain
    HTTP**; doctor writes `tls` on for every service it wires. Any real
    deployment runs doctor, so any real deployment is TLS — one config, one
    doctor run.
...
- **[#524](https://github.com/ivonnyssen/rusty-photon/issues/524) is superseded
  on mechanism but vindicated on goal** — decision 10 arrives at TLS-on, by a
  different route (doctor, not postinst) and on a corrected premise (four
  Alpaca drivers had no `tls` field to re-default). Its separable auth half
  survives and gets sharper — see below.
- **`rp` loses `init-tls` and gains nothing**; its BDD features
  (`acme_setup.feature`, `tls_setup.feature`) and `bdd-infra`'s one-shot
  command tests move with it. A breaking CLI change, sanctioned pre-1.0.
  Whether `rp hash-password` follows is **open**: it is auth rather than TLS,
  but by the same logic (credential material is service config) it belongs in
  doctor too.
...
- **Decision 8 raises the stakes on sentinel's unauthenticated restart
  endpoint.** Today it is effectively decorative on packaged Linux because the
  privilege path is missing. Once #523's polkit rule ships and supervision is
  universal, it becomes a working, LAN-reachable control that can restart the
  entire observatory. Decision 10 now puts TLS in front of it, which fixes the
  eavesdropping half — but **auth remains off**, so it is still unauthenticated.
  TLS-on does make required auth coherent for the first time (ADR-003's scheme
  is *"Basic Auth over TLS"*, and its own table rates cleartext Basic as
  providing no secrecy, no replay protection, and no tamper protection), so
  auth-by-default is now a live option rather than theatre.
