...
* To update the runner toolchain (new SDK pin, new runner release), boot a
  ...
  * **A Linux template rebuild must run a `bazel coverage //...` warmup before
    capture, not just a build/test warmup.** Coverage pulls a *nightly* rustc
    (`--config=coverage`) that the build/test toolchain never fetches, so a
    template warmed only with `bazel build`/`bazel test` still hands every
    coverage clone a cold nightly toolchain to fetch over WAN on its single
    job — measured at ~12 min cold versus ~2m47s once the toolchain is baked
    into the output base. The warmup must check out to the **exact** runner
    work path (`/home/ci/actions-runner/_work/rusty-photon/rusty-photon`):
    external repos — the toolchain among them — live under an output-base dir
    keyed by an md5 of the workspace path, so a warmup done at any other path
    populates a directory no clone will read. (Linux action keys are
    path-independent — they carry no `GITHUB_WORKSPACE`, unlike Windows — so
    the *build/test* actions transfer regardless of warmup path; only the
    external-repo fetch is path-sensitive, and that is what the nightly
    coverage toolchain is.) Run the warmup as the `ci` user against the
    runner's own `--config=remote-cache` + LAN-cache flags so it doubles as a
    full-job validation, then re-wipe `/etc/machine-id` as above.
