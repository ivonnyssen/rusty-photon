...
* To update the runner toolchain (new SDK pin, new runner release), boot a
  ...
  * **A Linux template rebuild must run a `bazel coverage --config=coverage
    //...` warmup before capture, not just a build/test warmup.** The
    `--config=coverage` flag is load-bearing: it is what overrides the default
    stable channel (`.bazelrc` pins `channel=stable`) to the pinned *nightly*
    toolchain — a bare `bazel coverage //...` compiles on stable and fetches
    nothing new, defeating the warmup. The build/test toolchain never pulls
    that nightly, so a template warmed only with `bazel build`/`bazel test`
    still hands every coverage clone a cold nightly toolchain to fetch over
    WAN on its single job — measured at ~12 min cold versus ~2m47s once the
    toolchain is baked into the output base. The warmup must check out to the
    **exact** runner work path
    ...
    Run it as the `ci` user with the runner's own CI flags — `bazel coverage
    --config=ci --config=coverage --config=remote-cache //...` plus the
    LAN-cache override — so the warmup doubles as a full-job validation, then
    re-wipe `/etc/machine-id` as above.
