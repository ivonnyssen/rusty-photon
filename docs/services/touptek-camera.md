...
| **Real binary** | The only target that actually links `libtoupcam` — the production `touptek-camera` `rust_binary` — is **deliberately not defined yet**. | Defining it now would make `bazel build //...` fail to link locally with no SDK. It lands once the **drafted** `.github/actions/install-toupcam-sdk` (INDI-mirror blobs + a parameterized Windows URL; a real aarch64 `cargo` link is already verified against it) is wired into a real-link workflow in **Phase F**. |
