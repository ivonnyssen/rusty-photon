...
| [svbony-camera](services/svbony-camera.md) | Camera — SVBony hardware | 11125 | v0 implemented 2026-07-21 — `Device` + full `Camera` over `svbony-rs`'s soft-trigger video-capture state machine; 65 unit tests + 60/60 BDD scenarios green, full quality gate green, ConformU wired (`tests/conformu_integration.rs` + a Bazel `conformu_integration` target, mirrors `zwo-camera`). Packaging landed: `rusty-photon-svbony-sdk-install` download-on-target helper (real pinned sha256 per arch, verified end-to-end), `postinst`/`postrm`, RUNPATH matching `zwo-camera`'s mechanism (`/usr/lib/rusty-photon/` is outside `ldconfig`'s default scan path). The real (non-simulation) `:svbony-camera` Bazel binary stays tagged `manual` and `scripts/build-packages.sh` has no SVBony SDK-staging leg yet (both deferred follow-up, not testable without the real SDK); **pending real-hardware validation** against a physical SV605CC (on order, not yet arrived). See `docs/services/svbony-camera.md` + `docs/plans/svbony-camera.md` + ADR-018. |
...
| [svbony-camera.md](plans/svbony-camera.md) | SVBony cooled-camera Alpaca driver (`svbony-camera`, port 11125; first hardware: SV605CC) + vendored `svbony-rs`/`libsvbony-sys` FFI. **v0-complete (Phases A-G landed 2026-07-21): pending real-hardware validation only** — SDK ships **no license** (→ a third ADR-013 bucket: download-on-target `.so`, packaging + the `rusty-photon-svbony-sdk-install` helper landed Phase G) and is **video-mode only** (exposures via soft trigger); API otherwise mirrors ZWO's, so `zwo-rs` was the structural template. Kept in this table (not archived) because real-hardware validation against the physical SV605CC — on order, not yet arrived — is still open in-scope work per `docs/skills/archiving-plans.md` |
| [qhyccd-convention-alignment.md](plans/qhyccd-convention-alignment.md) | Align the transcribed-from-hand-written `qhyccd-rs` to the `zwo-rs`/`svbony-rs` conventions: separates genuinely SDK-forced differences (f64/`u32::MAX` return convention, opaque-pointer handle, dual single-frame/live capture, SDK discriminant values — keep) from incidental historical divergence (handle model, `Control` representation, error shape, `mocks.rs`/`simulation/` layering, module split — unify). Phase 1 (correctness-first) reworks the handle model around the SDK-forced filter-wheel-through-camera-handle coupling. Not a shared-types-crate plan (that's follow-on). Draft, not started |
