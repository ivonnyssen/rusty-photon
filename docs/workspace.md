...
| [svbony-camera.md](plans/svbony-camera.md) | SVBony cooled-camera Alpaca driver (`svbony-camera`, port 11125; first hardware: SV605CC) + vendored `svbony-rs`/`libsvbony-sys` FFI. **COMPLETE: Phases A-G landed 2026-07-21, real-hardware validation PASSED 2026-07-26 (physical SV605CC, ConformU clean)** — SDK ships **no license** (→ a third ADR-013 bucket: download-on-target `.so`, packaging + the `rusty-photon-svbony-sdk-install` helper landed Phase G) and is **video-mode only** (exposures via soft trigger); API otherwise mirrors ZWO's, so `zwo-rs` was the structural template. Ready to archive per `docs/skills/archiving-plans.md` (deferred follow-ups — Bazel-side SDK-fetch rule, issue #710 dev-machine udev — are tracked outside the plan) |
