...
| [rusty-photon-camera-geometry](../crates/rusty-photon-camera-geometry/) | `crates/rusty-photon-camera-geometry` | Sensor-geometry arithmetic shared by the three ASCOM camera drivers: ROI validation and its rule order (R2/R3), the bin-ratio ROI rescale (B3), binned-full-frame sensor alignment (R4), `BayerOffsetX/Y` from a canonical mosaic (ST1), and `PercentCompleted`'s cap. Dependency-free by design — a vendor or `ascom-alpaca` type in a signature there means the item belongs in a driver, which is why each driver still translates its own SDK's Bayer spelling before asking for the offsets. Used by `qhy-camera`, `zwo-camera`, `svbony-camera`. |
