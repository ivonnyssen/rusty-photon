...
| [svbony-camera](services/svbony-camera.md) | Camera — SVBony hardware | 11125 | Phase C/D landed (2026-07-21): bare service skeleton over `svbony-rs`, `Device` real (config actions incl.), every `Camera` method an honest `NOT_IMPLEMENTED` stub. Registers the simulation backend's one fabricated `SV605CC-Simulated` camera only under `--features simulation`; the production build registers zero devices by default in this phase (deliberate boundary, not a technical one). Four of nine BDD feature files are genuinely green (`enumeration_connection`, `config_actions`, `auth`, `doctor`); five are `@wip` pending Phase E's soft-trigger video-capture exposure state machine. Native SVBony SDK dep, gated out of the default build; the real (non-simulation) `:svbony-camera` Bazel binary is tagged `manual` (cannot link — no `install-svbony-sdk` action exists yet and `SVBONY_SKIP_NATIVE_LINK=1` is baked in), so only the library and simulation-backed targets are first-class `//...` targets today. See `docs/services/svbony-camera.md` + ADR-018. |
