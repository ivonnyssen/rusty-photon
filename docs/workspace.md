...
| [touptek-camera](services/touptek-camera.md) | Camera — ToupTek (+ OEM-rebrand) hardware | 11123 | Phases A–E + Phase F gate landed: `touptek-rs`/`libtoupcam-sys` FFI + callback→blocking bridge, and the **full ASCOM `Camera`** over `touptek-rs` (trigger-mode exposure state machine, digital binning, ROI, gain/offset, cooling, RAW16 readout + transpose, sensor type, asynchronous ST4 PulseGuide) on `:11123`, driven by 60 green BDD scenarios + ~55 unit tests (the mock seam forces C2/C4/E9/GO1/K1/PG2), and **ConformU 0 errors / 0 issues** on the sim backend (wired into `conformu.yml`). **Build-gating win:** `libtoupcam` is a **dylib** (defers linking) and the `_sim` chain skip-links the SDK (`TOUPCAM_SKIP_NATIVE_LINK`), so the whole simulation `bazel test //...` is green **with no SDK provisioned** — unlike zwo/qhy. `install-toupcam-sdk` is wired into `native.yml`, which real-links on Linux/macOS (INDI mirror) and Windows (ToupTek's vendor SDK). The Bazel SDK-linking `rust_binary`, a Pi-aarch64 real-link leg, and real-hardware ConformU remain (need a provisioned SDK + hardware). No filter wheel/focuser (ToupTek ships none). One driver covers ~11 OEM rebrands (shared ABI). Licensing deferred. See `docs/services/touptek-camera.md` + `docs/plans/touptek-driver.md`. |
