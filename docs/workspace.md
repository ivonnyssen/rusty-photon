...
| [zwo-camera](services/zwo-camera.md) | Camera (+ FilterWheel) — ZWO ASI/EFW hardware | 11122 | Phase E (full Camera) landed: full `Device + Camera` over `zwo-rs` (exposure state machine, ROI/bin, gain/offset, cooling, readout, ST4 pulse-guiding), serial identity, config actions; 42 unit + 54 BDD green. Bazel first-class (`lib`/`binary`/`unit_test`; `bdd`/`conformu` build under Bazel, run under Cargo). EFW FilterWheel is Phase F; ConformU CI wiring is Phase G (the `zwo-rs` sim full-frame-fill speedup + writable `Exposure` control that previously tripped the timeout landed in rev `3c32e59`, so only the `conformu.yml` wiring remains). Native ZWO SDK dep, gated out of the default build. See `docs/services/zwo-camera.md` + ADR-008. |
