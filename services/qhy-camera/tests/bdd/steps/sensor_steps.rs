//! Sensor geometry and type steps.
...
#[then(regex = r"^camera device (\d+) reports a positive PixelSizeY$")]
async fn positive_pixel_size_y(world: &mut CameraWorld, _device: u32) {
    assert!(world.camera().pixel_size_y().await.unwrap() > 0.0);
}
