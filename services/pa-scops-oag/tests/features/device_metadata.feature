Feature: Device metadata
  ...
  Scenario: Device reports configured name
    Given a focuser service configured with name "Test Scops"
    Then the device name should be "Test Scops"
  ...
  Scenario: Device reports driver info naming Pegasus Scops and Focuser
    Given a running focuser service
    When I connect the device
    Then the driver info should contain "Pegasus"
    And the driver info should contain "Scops"
  ...
  Scenario: Max step matches configuration
    Given a focuser service configured with max step 80000
    When I connect the device
    Then the max step should be 80000

  Scenario: Max increment matches max step
    Given a focuser service configured with max step 80000
    When I connect the device
    Then the max increment should be 80000
