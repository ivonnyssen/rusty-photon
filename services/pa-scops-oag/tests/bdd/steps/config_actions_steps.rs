...
async fn running_focuser_service_with_port(world: &mut ScopsWorld, port: String) {
    ...
    let mut config = pa_scops_oag::Config::default();
...
async fn query_supported_actions(world: &mut ScopsWorld) {
...
async fn call_config_get(world: &mut ScopsWorld) {
    world.current_config().await;
}

#[when("config.schema is called")]
async fn call_config_schema(world: &mut ScopsWorld) {
    let focuser = Arc::clone(world.focuser());
    let body = focuser
...
async fn apply_max_step(world: &mut ScopsWorld, value: u32) {
...
async fn apply_pin_port_and_max_step(world: &mut ScopsWorld, value: u32) {
...
async fn apply_invalid_baud_rate(world: &mut ScopsWorld) {
    let mut config = world.current_config().await;
...
async fn call_named_action(world: &mut ScopsWorld, name: String) {
    let focuser = Arc::clone(world.focuser());
...
async fn assert_supported_actions(world: &mut ScopsWorld) {
    let actions = world
...
async fn assert_schema_sections(world: &mut ScopsWorld) {
...
async fn assert_schema_locked_field(world: &mut ScopsWorld) {
    let response = world.last_response.as_ref().expect("no response stashed");
...
async fn assert_schema_read_only_field(world: &mut ScopsWorld) {
    let response = world.last_response.as_ref().expect("no response stashed");
...
async fn assert_config_serial_port(world: &mut ScopsWorld, expected: String) {
    let response = world.last_response.as_ref().expect("no response stashed");
    assert_eq!(
...
async fn assert_no_overrides(world: &mut ScopsWorld) {
    let response = world.last_response.as_ref().expect("no response stashed");
...
async fn assert_apply_status(world: &mut ScopsWorld, expected: String) {
    let response = world.last_response.as_ref().expect("no response stashed");
    assert_eq!(response["status"].as_str(), Some(expected.as_str()));
}

#[then(regex = r"^the reload list should include (\S+)$")]
async fn assert_reload_includes(world: &mut ScopsWorld, path: String) {
    let response = world.last_response.as_ref().expect("no response stashed");
...
async fn assert_validation_errors(world: &mut ScopsWorld) {
    let response = world.last_response.as_ref().expect("no response stashed");
...
async fn assert_reloaded_serves(world: &mut ScopsWorld, expected: u32) {
    world.wait_for_config_max_step(expected).await;
}

#[then("the call should fail with an action-not-implemented error")]
async fn assert_action_not_implemented(world: &mut ScopsWorld) {
    assert_eq!(
