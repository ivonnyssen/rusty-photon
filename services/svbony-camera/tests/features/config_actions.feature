...
Feature: Configuration actions
  ...
  config. `config.apply` parses a full configuration blob: malformed input
  (wrong types, bad JSON) is rejected with an INVALID_VALUE error and leaves
  the file unchanged, while a valid change to the per-serial `devices` map
  is persisted atomically and applied through an in-process reload
  (`status:"applying"`). The actions work whether or not a device is
  connected -- and, unlike the exposure/ROI/gain/cooling surface, they are
  fully implemented as of Phase C/D (not `@wip`).
  ...
  Scenario: A per-serial device override is persisted and reloaded
    When config.apply sets the devices override "SVB0123456789AB" name to "Main Imaging"
