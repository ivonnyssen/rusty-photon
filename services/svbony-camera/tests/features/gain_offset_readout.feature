...
Feature: Gain, offset, and readout modes
  ...
  Setting an unknown index is rejected with INVALID_VALUE; changing the
  mode while an exposure is in flight is rejected with INVALID_OPERATION,
  so a delivered frame and the MaxADU describing it can never disagree.
