//! touptek-camera ASCOM Alpaca driver — CLI entry point.
...
use touptek_camera::{load_effective_config, CliOverrides, ServerBuilder};
use tracing::{debug, Level};

#[derive(Parser)]
#[command(name = "touptek-camera")]
#[command(about = "ASCOM Alpaca driver for ToupTek (and OEM-rebrand) cameras")]
#[command(version)]
struct Args {
    /// Path to the JSON config file. When omitted, resolves to the per-user
    /// platform config path (e.g. `~/.config/rusty-photon/touptek-camera.json` on
...
fn main() -> ServiceResult {
    ...
    let config_path = rusty_photon_config::resolve_config_path("touptek-camera", args.config)?;
    let overrides = CliOverrides { port: args.port };
    #[cfg(feature = "simulation")]
    let simulation_empty = args.simulation_empty;
    debug!(config = ?config_path, "starting touptek-camera");

    // No materialize_identity: ASCOM UniqueIDs are derived from the SDK device id
    // at enumeration, not minted into config (see the design doc "Device
    // identity").
    ...
    ServiceRunner::new("touptek-camera")
        .with_reload()
        .run_with_reload(move |shutdown, reload| async move {
            loop {
                ...
                if reloaded.load(Ordering::SeqCst) {
                    debug!(
                        "reload signalled; rebuilding touptek-camera from the new configuration"
                    );
                    continue;
                }
                return Ok(());
