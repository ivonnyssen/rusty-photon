...
Feature: Configuration actions
  ...
  UniqueIDs are derived from the SDK device id rather than minted into config.
  There is no filterwheel section -- ToupTek ships no filter wheel in this SDK.
  ...
  unchanged, while a valid change to the per-id `devices` map is persisted
  atomically and applied through an in-process reload (`status:"applying"`).
  The actions work whether or not a device is connected.

  Background:
    Given a running touptek-camera service with the simulation backend
  ...
  Scenario: A per-id device override is persisted and reloaded
    When config.apply sets the devices override "sim-0" name to "Main Imaging"
    Then the apply status should be applying
    And the reload list should include devices

  Scenario: An empty device name is rejected and not persisted
    When config.apply sets the devices override "sim-0" name to an empty string
