//! ConformU compliance test for the touptek-camera Camera driver.
//!
//! Launches the production binary (built with `--features conformu`, which pulls
//! in the `simulation` backend so the SDK yields one `Simulated ToupTek Camera`)
...
async fn conformu_compliance_tests() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config_path = temp_dir.path().join("touptek-camera.json");
    // No `filterwheel` section — ToupTek ships no filter wheel. Port 0 → OS-assigned.
