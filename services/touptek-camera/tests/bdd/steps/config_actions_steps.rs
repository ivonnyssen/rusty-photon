...
async fn apply_device_name(world: &mut CameraWorld, id: String, name: String) {
    let mut config = world.config_get().await;
    config["devices"]
        .as_object_mut()
        .expect("config.devices is an object")
        .insert(id, serde_json::json!({ "name": name }));
    world.call_action("config.apply", &config.to_string()).await;
}

#[when(regex = r#"^config\.apply sets the devices override "([^"]+)" name to an empty string$"#)]
async fn apply_empty_device_name(world: &mut CameraWorld, id: String) {
    let mut config = world.config_get().await;
    config["devices"]
        .as_object_mut()
        .expect("config.devices is an object")
        .insert(id, serde_json::json!({ "name": "" }));
    world.call_action("config.apply", &config.to_string()).await;
}
