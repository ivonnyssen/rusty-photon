//! Cucumber `World` for the touptek-camera BDD suite.
//!
...
impl CameraWorld {
    fn write_config(&mut self) -> String {
        // No `filterwheel` section — ToupTek ships no filter wheel in this SDK.
        ...
        let path = dir.path().join("touptek-camera.json");
        std::fs::write(
            &path,
    ...
    async fn acquire(&mut self) {
        ...
        if !self.empty_backend {
            panic!("touptek-camera did not register a Camera device within 20s");
        }
    }
