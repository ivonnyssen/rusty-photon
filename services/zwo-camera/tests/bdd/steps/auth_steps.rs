//! BDD step definitions for the zwo-camera TLS + HTTP Basic Auth smoke test.
...
fn generate_tls_certs(world: &mut CameraWorld) {
    ...
    rp_tls::cert::generate_service_cert(&ca_pem, &ca_key, "zwo-camera", &[], &certs_dir).unwrap();
    world.tls_pki_dir = Some(dir);
}

#[given("zwo-camera is configured with TLS and auth enabled and the simulation backend")]
fn configured_with_tls_and_auth(world: &mut CameraWorld) {
    ...
    world.pending_config = Some(serde_json::json!({
        ...
        "server": {
            ...
            "port": 0,
            "tls": {
                "cert": certs_dir.join("zwo-camera.pem").to_string_lossy().to_string(),
                "key": certs_dir.join("zwo-camera-key.pem").to_string_lossy().to_string()
            },
