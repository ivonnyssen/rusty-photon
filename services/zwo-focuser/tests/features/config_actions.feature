...
Feature: Configuration actions
  ...
  UniqueIDs are derived from the EAF SDK serial rather than minted into
  config. `config.apply` parses a full configuration blob: a valid change
  to the per-serial `devices` map is persisted atomically and applied
  ...
  Scenario: The config actions are advertised
    When the supported actions are queried on focuser device 0
  ...
  Scenario: A per-serial device override is persisted and reloaded
    When config.apply sets the devices override "EAF-SIM" name to "Main Focuser"
  ...
  Scenario: An unknown action is not implemented
    When the action "config.frobnicate" is called on focuser device 0
