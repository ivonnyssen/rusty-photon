...
async fn test_pool_try_acquire_reuses_running_server() {
    ...
    // test_service rebinds to a fresh random port on reload (SIGHUP on unix,
    // named-pipe message on windows), so the reloaded guard should almost
    // always surface a different port than the original. Asserting inequality
    // catches a silent regression where reload returns the cached port
    // without actually re-binding.
    assert_ne!(
