...
impl Camera {
    ...
    ///
    /// The SDK scales this field by the gain register: the value it reports is
    /// the model's gain-0 figure divided by `10^(gain/200)` (ASI gain is in
    /// 0.1 dB units). An ASI1600 reads 4.96 e⁻/ADU at gain 0 and 0.00496 at
    /// gain 600; an ASI178 reads 0.916 at gain 0 and 0.0025816 at gain 510 —
    /// both measured against the hardware. It must therefore be read **live**
    /// rather than taken from [`Camera::info`], which is a snapshot of the
    /// gain the camera happened to hold when it was opened.
    ///
    /// # Errors
...
impl Camera {
    ...
    /// Mirrors the SDK's gain scaling: the model's gain-0 figure
    /// ([`CameraInfo::e_per_adu`], which the simulated camera carries at its
    /// gain-0 value) divided by `10^(gain/200)`.
